// $Id: test_float.h,v 1.9 2009/06/30 08:42:05 jmh Exp $
 ...
 *
 *  MODIFICATIONS:
 *     Jens Munk Hansen <jmh@nru.dk>
 *     - Added support for missing C99 compliance 
 *     - Added case for minus_infinity in test_float_is_infinite
 */
...
{
  //uint32_t mask_comb          = (uint32_t)(((int32_t)a)>>31);
  //int32_t retval = ( (-a) & mask_comb ) | ( a & (~mask_comb) );
  //return retval;
...
inline int test_float_is_infinite(float a)
{
    ...
    return ( A.u32 == kInfAsInt ) || ( A.u32 == mInfAsInt );  
}
#else
inline int test_float_is_infinite(float a);
#endif
...
#else
 extern int test_float_compare(float a, float b, int maxUlps);
#endif
